...
function createInstances() {
	...
	kubectl create -f ${MANIFESTS_DIR}/postgres.yaml -n ${NAMESPACE}
	sleep 10
	...
	do
		...
		kubectl create -f ${MANIFESTS_DIR}/service-monitor-${inst}.yaml -n ${NAMESPACE}
	done
	for(( inst=0; inst<"${SERVER_INSTANCES}"; inst++ ))
	do
		...
		kubectl create -f ${MANIFESTS_DIR}/quarkus-resteasy-hibernate-${inst}.yaml -n ${NAMESPACE}
	...
	#Expose the services
	SVC_LIST=($(kubectl get svc --namespace=${NAMESPACE} | grep "service" | grep "tfb-qrh" | cut -d " " -f1))
	for sv in "${SVC_LIST[@]}"
	do
		kubectl expose svc/${sv} --namespace=${NAMESPACE}
