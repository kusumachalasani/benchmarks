...
K_CPU=2
K_MEM=6144
CONTAINER_NAME="renaissance-server"
...
echo "YAML_NAME is .... " ${YAML_NAME}

if [[ -z "${YAML_NAME}" ]]; then
	APP_NAME="renaissance"
	DEPLOYMENT_NAME="renaissance-sample-0"
else
	APP_NAME="renaissance"${YAML_NAME}
	DEPLOYMENT_NAME="renaissance"${YAML_NAME}"-sample-0"
fi
...
#       TOTAL_INST=1
        TOTAL_ITR=1
        MONITORING_LOAD="monitoring_fixedload"
fi
...
RESULTS_DIR_ROOT=${RESULTS_DIR_PATH}/renaissance-${BENCHMARKS}-$(date +%Y%m%d%H%M)
mkdir -p ${RESULTS_DIR_ROOT}

#Adding 5 secs buffer to retrieve CPU and MEM info
#CPU_MEM_DURATION=`expr ${DURATION} + 5`
##  Hard coding it to run all micro-benches. Uncomment after this run
CPU_MEM_DURATION=1296000
#604800
...
function runIterations() {
	...
	do
		...
		if [ ${RE_DEPLOY} == "true" ]; then
			...
			${SCRIPT_REPO}/renaissance-deploy.sh --clustertype=${CLUSTER_TYPE} -s ${BENCHMARK_SERVER} -n ${NAMESPACE} -i ${SCALING} -g ${RENAISSANCE_IMAGE}  --cpureq=${CPU_REQ} --memreq=${MEM_REQ} --cpulim=${CPU_LIM} --memlim=${MEM_LIM} --envoptions="${ENV_OPTIONS}" --usertunables="${OPTIONS_VAR}" -b ${BENCHMARKS} -t ${BENCHMARK_DURATION} --yaml=${YAML_NAME} >> ${LOGFILE}
		fi
