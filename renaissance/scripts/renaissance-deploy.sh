...
if [[ -z "${YAML_NAME}" ]]; then
        ...
        APP_NAME="renaissance"${YAML_NAME}
fi
...
function createInstances() {
	...
	if [ -z "${YAML_NAME}" ]; then
		SVC_MONITOR="service-monitor"
		APP_YAML="renaissance"
	else
		APP_YAML="renaissance"${YAML_NAME}
		SVC_MONITOR="service-monitor"${YAML_NAME}
	fi

	echo "SVC_MONITOR= "${SVC_MONITOR}
	echo "APP_YAML=" ${APP_YAML}

	#Create the deployments and services
	for(( inst=0; inst<"${SERVER_INSTANCES}"; inst++ ))
	do
		sed "s/name: ${APP_NAME}/name: ${APP_NAME}-${inst}/g" ${MANIFESTS_DIR}/${SVC_MONITOR}.yaml > ${MANIFESTS_DIR}/${SVC_MONITOR}-${inst}.yaml
		sed -i "s/${APP_NAME}-app/${APP_NAME}-app-${inst}/g" ${MANIFESTS_DIR}/${SVC_MONITOR}-${inst}.yaml
		sed -i "s/${APP_NAME}-port/${APP_NAME}-port-${inst}/g" ${MANIFESTS_DIR}/${SVC_MONITOR}-${inst}.yaml
		${K_EXEC} create -f ${MANIFESTS_DIR}/${SVC_MONITOR}-${inst}.yaml -n ${NAMESPACE}
	done

	for(( inst=0; inst<"${SERVER_INSTANCES}"; inst++ ))
	do
		sed "s/${APP_NAME}-sample/${APP_NAME}-sample-${inst}/g" ${MANIFESTS_DIR}/${APP_YAML}.yaml > ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		sed -i "s|${BENCHMARK_IMAGE}|${RENAISSANCE_IMAGE}|g" ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		sed -i "s/${APP_NAME}-service/${APP_NAME}-service-${inst}/g" ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		sed -i "s/${APP_NAME}-app/${APP_NAME}-app-${inst}/g" ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		sed -i "s/${APP_NAME}-port/${APP_NAME}-port-${inst}/g" ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
	
		# Setting cpu/mem request limits
		if [ ! -z  ${MEM_REQ} ]; then
			sed -i '/requests:/a \ \ \ \ \ \ \ \ \ \ memory: '${MEM_REQ}'' ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		fi
		if [ ! -z  ${CPU_REQ} ]; then
			sed -i '/requests:/a \ \ \ \ \ \ \ \ \ \ cpu: '${CPU_REQ}'' ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		fi
		if [ ! -z  ${MEM_LIM} ]; then
			sed -i '/limits:/a \ \ \ \ \ \ \ \ \ \ memory: '${MEM_LIM}'' ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		fi
		if [ ! -z  ${CPU_LIM} ]; then
			sed -i '/limits:/a \ \ \ \ \ \ \ \ \ \ cpu: '${CPU_LIM}'' ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		fi
		if [ ! -z  ${BENCHMARKS} ]; then
			sed -i "/env:/a \ \ \ \ \ \ \ \ \ \ \ \ value: \"${BENCHMARKS}\""  ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
             		sed -i '/env:/a \ \ \ \ \ \ \ \ \ \ - name: "BENCHMARKS"' ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		fi
		echo "MESSAGE" ${BENCHMARKS}
		if [ ! -z  ${BENCHMARK_DURATION} ]; then
			sed -i "/env:/a \ \ \ \ \ \ \ \ \ \ \ \ value: \"${BENCHMARK_DURATION}\"" ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
             		sed -i '/env:/a \ \ \ \ \ \ \ \ \ \ - name: "TIME_LIMIT"' ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		fi
		...
		if [ ! -z  "${OPTIONS_VAR}" ]; then
			sed -i "s/\"-server\"/\"${OPTIONS_VAR}\"/" ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
		fi
		
		if [ ! -z  "${JDK_JAVA_OPTIONS}" ]; then
                        sed -i "/env:/a \ \ \ \ \ \ \ \ \ \ \ \ value: \"${JDK_JAVA_OPTIONS}\"" ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
                        sed -i '/env:/a \ \ \ \ \ \ \ \ \ \ - name: "JDK_JAVA_OPTIONS"' ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml
                fi

		
		${K_EXEC} create -f ${MANIFESTS_DIR}/${APP_YAML}-${inst}.yaml -n ${NAMESPACE}
...
function stopAllInstances() {
	${RENAISSANCE_REPO}/renaissance-cleanup.sh -c ${CLUSTER_TYPE} -n ${NAMESPACE} -y ${YAML_NAME} >> ${LOGFILE}
	sleep 30
